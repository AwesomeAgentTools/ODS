...
assert_contains "installers/macos/install-macos.sh" '\[\[ -z "\$platform" \]\] && docker image inspect "\$image"' "macOS pre-pull may trust a cached image with the wrong architecture"
