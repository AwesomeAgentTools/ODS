...
def magic_link_module(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(ml, "_ods_proxy_lan_ready", lambda: (True, ""))
...
def test_generate_respects_public_url_env(
    ...
    monkeypatch.setenv("ODS_PUBLIC_URL", "http://ods.local:3002")
    resp = magic_link_client.post(
        "/api/auth/magic-link/generate",
    ...
    assert resp.json()["url"].startswith("http://ods.local:3002/auth/magic-link/")


def test_generate_url_defaults_to_auth_subdomain(magic_link_client):
    """With no ODS_PUBLIC_URL set, the URL builds off the auth subdomain
    (http://auth.<ODS_DEVICE_NAME>.local) — that's where ods-proxy
    ...
    assert resp.status_code == 200
    url = resp.json()["url"]
    assert url.startswith("http://auth.ods.local/magic-link/"), url
...
def test_generate_strips_trailing_slash_from_public_url(
    ...
    assert url.startswith("http://ods.local/auth/magic-link/")
...
def test_generate_owner_lan_requires_ods_proxy(
    ...
    monkeypatch.setattr(
        ...
        lambda: (False, "ODS Talk owner cards require ods-proxy"),
    )
    ...
    assert "ods-proxy" in resp.json()["detail"]
...
def test_owner_public_url_mode_does_not_require_ods_proxy(
    ...
    assert resp.status_code == 200, resp.text
    assert resp.json()["url"].startswith("https://ods.example/auth/magic-link/")
...
def test_owner_card_status_reports_proxy_state(
    ...
    monkeypatch.setattr(
        magic_link_module,
        "_ods_proxy_lan_ready",
        lambda: (False, "ods-proxy is not enabled"),
    )
...
def test_ods_proxy_service_refreshes_stale_manifest_cache(
    ...
    ods-proxy` renames compose.yaml.disabled to compose.yaml, the running
    ...
    service = magic_link_module._ods_proxy_service()

    assert service == refreshed["ods-proxy"]
    assert magic_link_module.SERVICES["ods-proxy"] == refreshed["ods-proxy"]


def test_ods_proxy_service_keeps_disabled_state(magic_link_module, monkeypatch):
    magic_link_module.SERVICES.clear()
...
def test_redeem_issues_signed_cookie_that_verifies(
    ...
    assert cookie, "redemption did not set ods-session cookie"
...
def test_owner_token_can_be_redeemed_repeatedly_and_revoked(
    ...
    monkeypatch.setenv("ODS_DEVICE_NAME", "studio")
...
def test_redeem_refuses_when_signing_unconfigured(magic_link_client, magic_link_module):
    """If ODS_SESSION_SECRET isn't configured, redemption returns 503
...
def test_qr_endpoint_returns_data_url_when_qrcode_installed(magic_link_client):
    ...
    resp = magic_link_client.get(
        "/api/auth/magic-link/qr?url=http://ods.local:3002/auth/magic-link/abc",
        headers=magic_link_client.auth_headers,
    )
